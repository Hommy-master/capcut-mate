...
| 字段名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| alpha | number | ❌ | 1.0 | 阴影不透明度，取值范围为[0, 1] |
| color | string | ❌ | "#000000" | 阴影颜色（十六进制） |
| diffuse | number | ❌ | 15.0 | 阴影扩散程度，取值范围为[0, 100] |
| distance | number | ❌ | 5.0 | 阴影距离，取值范围为[0, 100] |
| angle | number | ❌ | -45.0 | 阴影角度，取值范围为[-180, 180] |
