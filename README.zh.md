...
从 [Releases](https://github.com/Hommy-master/capcut-mate/releases) 下载 `.dmg` 后，双击安装或首次打开时，macOS 可能弹出：

> **「"剪映小助手"已损坏，无法打开。你应该将它移到废纸篓。」**

**别担心，软件没有真的损坏，也不要点「移到废纸篓」！** 这是 macOS 对未签名应用的误导性提示。按下面步骤处理一次即可：

> **原理：** 清除 macOS 给文件打的「隔离标记」，之后就能正常安装和打开。
...
| 4 | 按 **回车** 执行命令 |
| 5 | 再去**双击** `.dmg`，挂载后把应用拖到「**应用程序**」文件夹，然后打开 |

**已经安装好了，打开时还是报同样错误？** 重复步骤 2–4，但这次把 `/Applications/剪映小助手.app` 拖进终端即可。
