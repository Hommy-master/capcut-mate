...
def test_keyframes_infos_always_normalizes_small_pixel_values():
    assert normalize_keyframe_value(
        "KFTypePositionX", 1.0, width=1920, assume_pixel=True
    ) == 1.0 / 1920


def test_rotation_scale_alpha_unchanged_even_when_abs_gt_one():
    """旋转/缩放/透明度不应做画布尺寸归一化（值域与位置不同）。"""
    canvas = dict(width=1920, height=1080)
    assert normalize_keyframe_value("KFTypeRotation", 90.0, **canvas) == 90.0
    assert normalize_keyframe_value("KFTypeRotation", -180.0, **canvas) == -180.0
    assert normalize_keyframe_value("UNIFORM_SCALE", 1.3, **canvas) == 1.3
    assert normalize_keyframe_value("UNIFORM_SCALE", 2.0, **canvas) == 2.0
    assert normalize_keyframe_value("KFTypeAlpha", 0.5, **canvas) == 0.5
    assert normalize_keyframe_value("KFTypeAlpha", 0.0, **canvas) == 0.0


def test_rotation_scale_alpha_unchanged_with_assume_pixel():
    """keyframes_infos 的 assume_pixel 也不应误伤非位置类型。"""
